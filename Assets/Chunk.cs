		...
		for(int z = 0; z < World.chunkSize; z++)
			for(int y = 0; y < World.chunkSize; y++)
				for(int x = 0; x < World.chunkSize; x++)
				{
                    ...
                    //create a series of variables to store our world position within the chunks.
                    int worldX = (int)(x + chunk.transform.position.x);
                    int worldY = (int)(y + chunk.transform.position.y);
                    int worldZ = (int)(z + chunk.transform.position.z);
                    //compare the world position to the Generated Perlin Noise map height position and create Air if above that height value.
                    if (Utils.fBM3D(worldX, worldY, worldZ, 0.01f, 2) <.45f)
                    {
